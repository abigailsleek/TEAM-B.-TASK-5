...
Basically clubbing just makes the work easier by merging the filtered and stem text together as one instead
of creating a TF-IDF vector simultaneously for the stem and filtered text, it's done just once on the clubbed data."""
...
*  ** TF-TDF VECTORIZER**
You know computers are not naturally smart except we make to and they don't understand texts only 0's and 1's.
So we are going to need to transform the texts to numbers
...
from the dendrogram above, you will notice that based on the distance calculation gotten from cosine _similarity,
we have been able to cluster similar movies. 
Therefore, supposing i am searching for a similar movie after Fargo, it definately will be Northwest, or if you
look up The Gradute, a similar movie to it will be A Clockwork Orange. That is how movie similarity works using the
above clustering methods."""
